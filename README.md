...
**Operational Analytics & Digital Process Analyst.** Data, processes, systems
and people, connected until something measurably improves.

I build analytics that survive being questioned: every published figure
reconciles back to its source, every rule that removed a row is named, and
every recommendation carries a number and an owner.
...
| [**problem-radar**](https://github.com/DataVantage/AI_Problem-Radar) | *"Which concrete, budgeted business problem is worth starting with?"* Collects public signals, deduplicates, clusters and scores them — every score explainable down to the sentence that produced it. | Python (standard library only) · TF-IDF · SQLite |

---

### How I work

- **Reconciliation is a gate, not a slide.** If a slice doesn't add up to the
  headline, the pipeline stops instead of publishing.
- **Data quality is governed, not cleaned up.** Rules are declarative,
  reviewable by non-programmers, and the reason a row was excluded stays
  attached to it.
- **Nothing invented.** What isn't evidenced is labelled unknown — in the code,
  in the report and in the dashboard.
- **The dashboard is the last step, not the first.** Define the question,
  define the numbers, make them trustworthy, model them, *then* visualise.

---

### Toolbox

**Analysis & BI** — SQL (MS SQL Server, SQLite, DuckDB) · Power BI · DAX ·
Power Query · Excel · Python (pandas, numpy) · R
...
*Based in Germany · open to roles in analytics, BI and operations.*
